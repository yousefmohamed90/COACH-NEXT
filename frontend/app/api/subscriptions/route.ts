...
export async function GET(request: NextRequest) {
  ...
  const subs = await getDb().select().from(subscriptionsTable).where(eq(subscriptionsTable.coachId, session.userId));
...
export async function POST(request: NextRequest) {
  ...
  const [sub] = await getDb().insert(subscriptionsTable).values({
